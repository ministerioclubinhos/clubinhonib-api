...
export class ClubControlController {
  ...
  async createPeriod(@Body() dto: CreateClubPeriodDto) {
    ...
    try {
      ...
      this.logger.error(
        `POST /club-control/periods -> error in ${Date.now() - started}ms: ${error?.message}`,
      );
      throw err;
    }
  }
  ...
  async getPeriodByYear(@Param('year') year: number) {
    ...
    try {
      ...
      const error = err as Error;
      this.logger.error(
        `GET /club-control/periods/${year} -> error in ${Date.now() - started}ms: ${error?.message}`,
      );
      throw err;
    }
  }
  ...
  async getAllPeriods(
    ...
    try {
      ...
      const error = err as Error;
      this.logger.error(
        `GET /club-control/periods -> error in ${Date.now() - started}ms: ${error?.message}`,
      );
      throw err;
    }
  }
  ...
  async updatePeriod(
    ...
    try {
      ...
      if (!(err instanceof NotFoundException)) {
        const error = err as Error;
        this.logger.error(
          `PUT /club-control/periods/${id} -> error in ${Date.now() - started}ms: ${error?.message}`,
        );
      }
      throw err;
    }
  ...
  async deletePeriod(@Param('id') id: string) {
    ...
    try {
      ...
      if (!(err instanceof NotFoundException)) {
        const error = err as Error;
        this.logger.error(
          `DELETE /club-control/periods/${id} -> error in ${Date.now() - started}ms: ${error?.message}`,
        );
      }
      throw err;
    }
  ...
  async createException(@Body() dto: CreateClubExceptionDto) {
    ...
    try {
      ...
      const error = err as Error;
      this.logger.error(
        `POST /club-control/exceptions -> error in ${Date.now() - started}ms: ${error?.message}`,
      );
      throw err;
    }
  }
  ...
  async getExceptionByDate(@Param('date') date: string) {
    ...
    try {
      ...
      const error = err as Error;
      this.logger.error(
        `GET /club-control/exceptions/${date} -> error in ${Date.now() - started}ms: ${error?.message}`,
      );
      throw err;
    }
  }
  ...
  async getExceptionsByPeriod(
    ...
    try {
      ...
      const error = err as Error;
      this.logger.error(
        `GET /club-control/exceptions -> error in ${Date.now() - started}ms: ${error?.message}`,
      );
      throw err;
    }
  }
  ...
  async deleteException(@Param('id') id: string) {
    ...
    try {
      ...
      if (!(err instanceof NotFoundException)) {
        const error = err as Error;
        this.logger.error(
          `DELETE /club-control/exceptions/${id} -> error in ${Date.now() - started}ms: ${error?.message}`,
        );
      }
      throw err;
    }
  ...
  async checkClubWeek(
    ...
    try {
      ...
      const error = err as Error;
      this.logger.error(
        `GET /club-control/check/club/${clubId} -> error in ${Date.now() - started}ms: ${error?.message}`,
      );
      throw err;
    }
  }
  ...
  async checkAllClubsWeek(
    ...
    try {
      ...
      const listed = Array.isArray(result.clubs) ? result.clubs.length : 0;
      const periodYear = (result.period as { year?: number })?.year ?? 0;
      const periodWeek = (result.period as { week?: number })?.week ?? 0;
      this.logger.log(
        `GET /club-control/check/week -> success in ${Date.now() - started}ms clubsListed=${listed} total=${result.summary?.totalClubs ?? 0} year=${periodYear} week=${periodWeek}`,
      );
      return result;
    } catch (err) {
      const error = err as Error;
      this.logger.error(
        `GET /club-control/check/week -> error in ${Date.now() - started}ms: ${error?.message}`,
      );
      throw err;
    }
  }
  ...
  async getCurrentWeekDashboard() {
    ...
    try {
      ...
      const error = err as Error;
      this.logger.error(
        `GET /club-control/dashboard -> error in ${Date.now() - started}ms: ${error?.message}`,
      );
      throw err;
    }
  }
  ...
  async getCurrentWeek() {
    ...
    try {
      ...
      const error = err as Error;
      this.logger.error(
        `GET /club-control/current-week -> error in ${Date.now() - started}ms: ${error?.message}`,
      );
      throw err;
    }
  }
  ...
  async getDetailedIndicators(
    ...
    try {
      ...
      )) as { clubs?: { critical?: unknown[]; warning?: unknown[] } };
      const critical = (result?.clubs?.critical?.length as number) || 0;
      ...
      const error = err as Error;
      this.logger.error(
        `GET /club-control/indicators/detailed -> error in ${Date.now() - started}ms: ${error?.message}`,
      );
      throw err;
    }
  }
