...
// src/modules/clubs/repositories/clubs.repository.ts (apenas o método findAllPaginated)
...
async findAllPaginated(
  ...
  // 3) Clube (número ou weekday por texto)
...
async createClub(dto: CreateClubDto): Promise<ClubEntity> {
  return this.dataSource.transaction(async (manager) => {
    ...
    // 2) Coordinator (opcional + verificação de existência)
    ...
    // 4) Professores (opcional): validar existência e vinculação
    if (dto.teacherProfileIds?.length) {
      const ids = Array.from(new Set(dto.teacherProfileIds)); // de-dup
      ...
      if (alreadyAssigned.length) {
        // lista apenas os ids (ou adapte para nome/email se quiser)
      ...
      // vincula todos ao novo clube
      await teacherRepo.update({ id: In(ids) }, { club: { id: club.id } as any });
    }

    // 5) Retorna com joins completos, mesma transação
  ...
  async list(): Promise<ClubSelectOptionDto[]> {
    const items = await this.clubRepo.find({
      relations: { address: true, coordinator: true },
      order: { number: 'ASC' },
    });
