...
export class ImagePageGetService {
  ...
  async findAll(): Promise<ImagePageResponseDto[]> {
    ...
    const result = pages.map((page) =>
      ImagePageResponseDto.fromEntity(page, mediaMap),
    );
  ...
  async findOne(id: string): Promise<ImagePageResponseDto> {
    ...
    if (!page) {
      ...
      throw new AppNotFoundException(
        ...
        'Página de galeria não encontrada.',
      );
    }
    this.logger.debug(
      `✅ Página encontrada: ID=${page.id}, name="${page.name}"`,
    );
    ...
    if (!page.route) {
      ...
      throw new AppNotFoundException(
        ErrorCode.RESOURCE_NOT_FOUND,
        `A galeria com id ${id} não possui rota associada.`,
      );
    }
