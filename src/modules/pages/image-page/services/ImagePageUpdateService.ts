...
export class ImagePageUpdateService {
  ...
  async updateImagePage(
    ...
    this.logger.log(
      `🚀 Iniciando atualização da página de imagens com ID: ${id}`,
    );
    ...
    try {
      ...
      this.logger.debug(
        `🔍 Validando seções da página ID: ${imagePageExisting.id}`,
      );
      const imageSectionExisting = await this.validateSections(
        imagePageExisting.id,
      );
      this.logger.debug(
        `🔍 Validando rota da página ID: ${imagePageExisting.id}`,
      );
      const imagePageRouteExisting = await this.validateRoute(
        imagePageExisting.id,
      );
      this.logger.debug(`🔍 Validando mídias das seções`);
      const oldMedia = await this.validateMedia(
        imageSectionExisting.map((section) => section.id),
      );

      this.logger.debug(`🗑️ Iniciando exclusão de seções obsoletas`);
      await this.deleteSections(
        imageSectionExisting,
        pageData.sections,
        queryRunner,
      );
      ...
      const savedImagePage = await this.upsertImagePage(
        imagePageExisting.id,
        pageData,
        queryRunner,
      );
      ...
      for (const sectionInput of pageData.sections) {
        this.logger.debug(
          `📝 Processando seção: caption="${sectionInput.caption}", id=${sectionInput.id || 'novo'}`,
        );
        ...
        this.logger.debug(
          `🖼️ Processando mídias da seção ID: ${savedSection.id}`,
        );
      ...
      const finalImagePage = await queryRunner.manager.save(
        ImagePageEntity,
        savedImagePage,
      );
      ...
      this.logger.log(
        `✅ Página de imagens atualizada com sucesso: ID=${finalImagePage.id}`,
      );
  ...
  async validateImagePage(id: string): Promise<ImagePageEntity> {
    ...
    this.logger.debug(
      `✅ Galeria validada: ID=${imagePage.id}, name="${imagePage.name}"`,
    );
    return imagePage;
  }
  ...
  async validateSections(pageId: string): Promise<ImageSectionEntity[]> {
    ...
    if (!sections || sections.length === 0) {
      ...
      throw new AppNotFoundException(
        ErrorCode.RESOURCE_NOT_FOUND,
        'Seções da galeria não encontradas',
      );
    }
    this.logger.debug(
      `✅ Seções validadas: ${sections.length} seções encontradas`,
    );
    return sections;
  }
  ...
  async validateRoute(entityId: string): Promise<RouteEntity> {
    ...
    if (!route) {
      ...
      throw new AppNotFoundException(
        ErrorCode.RESOURCE_NOT_FOUND,
        'Rota da galeria não encontrada',
      );
    }
  ...
  async validateMedia(sectionIds: string[]): Promise<MediaItemEntity[]> {
    ...
    if (!media || media.length === 0) {
      ...
      throw new AppNotFoundException(
        ErrorCode.RESOURCE_NOT_FOUND,
        'Mídias associadas à galeria não encontradas',
      );
    }
    this.logger.debug(
      `✅ Mídias validadas: ${media.length} mídias encontradas`,
    );
    return media;
  }
  ...
  async upsertImagePage(
    ...
    const savedImagePage = await queryRunner.manager.save(
      ImagePageEntity,
      imagePageToUpsert,
    );
    this.logger.debug(
      `✅ Galeria upsertada: ID=${savedImagePage.id}, name="${savedImagePage.name}"`,
    );
  ...
  async addSection(
    ...
    this.logger.debug(
      `🆕 Preparando adição de nova seção: caption="${sectionInput.caption}"`,
    );
  ...
  async deleteSections(
    ...
    this.logger.debug(
      `🗑️ ${sectionsToRemove.length} seções marcadas para remoção`,
    );
    for (const section of sectionsToRemove) {
      this.logger.debug(
        `🗑️ Removendo seção ID: ${section.id}, caption="${section.caption}"`,
      );
  ...
  async deleteMedia(
    ...
    for (const media of mediaToRemove) {
      ...
      this.logger.debug(
        `🗑️ Removendo mídia ID: ${media.id}, URL="${media.url || 'não fornecida'}"`,
      );
  ...
  async upsertSection(
    ...
    this.logger.debug(
      `🔄 Preparando upsert de seção ID: ${sectionInput.id}, caption="${sectionInput.caption}"`,
    );
  ...
  async upsertMedia(
    ...
    this.logger.debug(
      `🔄 Iniciando upsert de mídia: ID=${mediaInput.id}, fieldKey="${mediaInput.fieldKey || 'não fornecido'}"`,
    );
    this.logger.debug(
      `📋 Construindo base da mídia para targetId: ${targetId}`,
    );
  ...
  async upsertRoute(
    ...
    const routeData: Partial<RouteEntity> = {
      ...
      image:
        'https://clubinho-nib.s3.us-east-1.amazonaws.com/production/cards/card_imagens.png',
      public: pageData.public,
    };
  ...
  async processSectionMedia(
    ...
    for (const mediaInput of mediaItems) {
      this.logger.debug(
        `📽️ Processando mídia: id=${mediaInput.id || 'novo'}, fieldKey="${mediaInput.fieldKey || 'não fornecido'}"`,
      );
      if (mediaInput.id) {
    ...
    this.logger.debug(
      `✅ Processamento concluído: ${processedMedia.length} mídias`,
    );
