...
export class ImagePageCreateService {
  ...
  async createImagePage(
    ...
    this.logger.log(
      `🚀 Iniciando criação de página de imagens com título: "${pageData.title}"`,
    );
    ...
    try {
      ...
      this.logger.debug(
        `🛤️ Gerando caminho para a rota com prefixo "galeria_imagens_"`,
      );
      ...
      for (const sectionInput of sections) {
        this.logger.debug(
          `📝 Criando seção: caption="${sectionInput.caption}"`,
        );
        ...
        const mediaItemsPrepared = sectionInput.mediaItems.map((item) => {
          ...
          this.logger.debug(
            `📋 Item de mídia preparado: fieldKey="${item.fieldKey || 'não fornecido'}"`,
          );
          return {
            ...item,
        ...
        this.logger.debug(
          `📤 Processando itens de mídia para seção ID: ${savedSection.id}`,
        );
        const mediaItems =
        ...
        this.logger.debug(
          `✅ ${mediaItems.length} itens de mídia processados para seção ID: ${savedSection.id}`,
        );
      }
      ...
      this.logger.log(
        `✅ Página de imagens criada com sucesso: ID=${finalGallery.id}`,
      );
      ...
      throw new BadRequestException(
        'Erro ao criar a galeria. Nenhum dado foi salvo.',
      );
    } finally {
